...
import functools
import logging
from data.resource import DataBase
...
class StockList(DataBase):
    ...
    def __init__(self, logger_dir="resource_data", logger_name=datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")):
    ...
    def sqlGtPrice(value):
        return DataBase.sqlGt(key="CAST(PRICE as demical)", value=value)

    @staticmethod
    def sqlGePrice(value):
        return DataBase.sqlGe(key="CAST(PRICE as demical)", value=value)

    @staticmethod
    def sqlEqPrice(value):
        return DataBase.sqlEq(key="CAST(PRICE as demical)", value=value)

    @staticmethod
    def sqlNePrice(value):
        return DataBase.sqlNe(key="CAST(PRICE as demical)", value=value)

    @staticmethod
    def sqlLePrice(value):
        return DataBase.sqlLe(key="CAST(PRICE as demical)", value=value)

    @staticmethod
    def sqlLtPrice(value):
        return DataBase.sqlLt(key="CAST(PRICE as demical)", value=value)
    ...
    def add_(self, table_name=None, primary_column: str = None, values: list = None):
        super().add_(primary_column="STOCK_ID", values=values)

    def select(self, table_name: str = None, columns: list = None, where: list = None,
    ...
    def selectByPriceRange(self, price_range: tuple, contain_price: bool = False,
        ...
        """
        # price_range[0] <= CAST(PRICE as demical) AND CAST(PRICE as demical) < price_range[1]
        price_filter = DataBase.sqlAnd(StockList.sqlGePrice(price_range[0]),
                                       StockList.sqlLtPrice(price_range[1]))
    ...
    def selectByStockIds(self, stock_ids: list = None,
                         sort_by: str = "CAST(PRICE as demical)", sort_type="ASC"):
        """
...
if __name__ == "__main__":
    class StockListTester:
        ...
        def selectByStockIds(self):
            from data import Investment
            inventory = Investment()
            results = self.stock_list.selectByStockIds(stock_ids=inventory.getInventory())
            # result = self.stock_list.selectByStockIds(stock_ids=["00739"])
